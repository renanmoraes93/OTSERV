...
void Game::playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t itemId, uint16_t amount, uint64_t price, uint8_t tier, bool anonymous) {
	...
	if (type == MARKETACTION_SELL) {
      ...
      //CoinType::Normal
      player->getAccount()->removeCoins(enumToValue(CoinType::Normal), static_cast<uint32_t>(amount), "");
		} else {
...
void Game::playerCancelMarketOffer(uint32_t playerId, uint32_t timestamp, uint16_t counter) {
	...
	if (offer.type == MARKETACTION_BUY) {
      ...
      //CoinType::Normal
      player->getAccount()->addCoins(enumToValue(CoinType::Normal), offer.amount, "");                  
...
void Game::playerAcceptMarketOffer(uint32_t playerId, uint32_t timestamp, uint16_t counter, uint16_t amount) {
	...
	if (offer.type == MARKETACTION_BUY) {
      ...
      //CoinType::Normal
      buyerPlayer->getAccount()->addCoins(enumToValue(CoinType::Normal), amount, "Purchased on Market");            
		} else if (it.stackable) {
      ...
      //CoinType::Normal
      player->getAccount()->addCoins(enumToValue(CoinType::Normal), amount, "Purchased on Market");            
		} else if (it.stackable) {
			uint16_t tmpAmount = amount;
			while (tmpAmount > 0) {
				uint16_t stackCount = std::min<uint16_t>(it.stackSize, tmpAmount);
