...
function wagon.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if count >= releaseCount then
		...
		if isPrisioned then
			player:sendTextMessage(MESSAGE_GAME_HIGHLIGHT, "[Anti-Bot] Voce completou sua sentenca na prisao! Tente nao ser preso novamente por cavebot afk.")
			...
			player:sendTextMessage(MESSAGE_GAME_HIGHLIGHT, "You have been released from prison, lets try not to return again.")
		end
    ...
    player:sendTextMessage(MESSAGE_GAME_HIGHLIGHT, "You still need to mine "..releaseCount - count.." token(s) to be released.")
