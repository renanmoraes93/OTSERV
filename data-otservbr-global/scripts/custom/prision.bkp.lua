local prison = {
    ...
    templePosition = Position(31575, 32596, 7), -- currently Thais Temple
...
function prisonTalkaction.onSay(player, words, param)
    if player:getGroup():getAccess() and param ~= "" then
        ...
        targetPlayer:sendTextMessage(MESSAGE_GAME_HIGHLIGHT, "You have been jailed by AutomaticSystem.")
        player:sendTextMessage(MESSAGE_GAME_HIGHLIGHT, "You have jailed " .. targetPlayer:getName() .. ".")
        targetPlayer:addItem(prison.pickId)
        Game.broadcastMessage(targetPlayer:getName() .. " has been imprisoned by AutomaticSystem", MESSAGE_STATUS_WARNING)
        return false
    end
end
...
function wagon.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if count >= releaseCount then
        ...
        player:sendTextMessage(MESSAGE_LOOK, "You have been released from prison, let's try not to return again.")
