...
function sellItemAction.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:hasExhaustion("loot-auto-seller") then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("You attempted to sell items. Please wait %d seconds before trying again.", saleSettings.cooldownToUse))
		return true
	end
	...
	local itemsInInbox = inbox:getItems(true)
	...
	if itemsSold > 0 then
		...
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("You sold %d items, and the value of %d gold has been added to your bank balance.", itemsSold, totalSellValue))
		player:setExhaustion("loot-auto-seller", saleSettings.cooldownToUse)
	else
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You don't have any items to sell.")
	end
	return true
end

sellItemAction:id(45218)
